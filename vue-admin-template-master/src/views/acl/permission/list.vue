<template>
  <div>
     <!-- 
      ...
      type: 2
    -->
     <el-table :data="menuPermissionList" border style="margin-bottom:20px" 
      :expand-row-keys="expandKeys" row-key="id">
        ...
        <el-table-column label="操作" width="width">
           <template slot-scope="row">
              <HintButton type="primary" icon="el-icon-plus" size="mini"
              :disabled="row.level===4" :title="getAddTitle(row.level)" @click="toAddPermission(row)"></HintButton>
              <HintButton type="primary" icon="el-icon-edit" size="mini" @click="toUpdatePermission(row)"
              :disabled="row.level===1" :title="row.level===4 ? '修改功能':'修改菜单'"></HintButton>
              <HintButton type="danger" icon="el-icon-delete" size="mini"
              :disabled="row.level===1" title="删除" @click="removePermission(row)"></HintButton>
           </template>
        </el-table-column>
     </el-table>

     <el-dialog :title="dialogTitle" :visible.sync="dialogPermissionVisible" :before-close="resetData">
        <el-form :model="permission" label-width="120px" ref="permission" :rules="permissionRules">
   ...
   methods: {
      ...
      removePermission(permission){
         this.$confirm('此操作将永久删除该记录, 是否继续?','提示',{
            ...
            let result = await this.$API.permission.removePermission(permission.id)
      ...
      addOrUpdatePermission(){
         this.$refs.permission.Validate(async (valid) => {
            if(valid){
               const {pname,...parm} = this.permission
               let result = await this.$API.permission[parm.id ? 'updatePermission':'addPermission'](parm)
               this.$message.success(result.message || `${parm.id ? '修改':'添加'}成功`)
               this.resetData()
               this.fetchPermissionList()
            }
         })
